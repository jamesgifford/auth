...
### Fixed
- `HasPublicId` generated `public_id` from a `creating` model event (`src/PublicId/Concerns/HasPublicId.php`), and that event is suppressed by any code path that turns off model events — `Model::withoutEvents()`, `saveQuietly()`, and a `DatabaseSeeder` using `Illuminate\Database\Console\Seeds\WithoutModelEvents` (present, commented, in Laravel's stock seeder stub, and enabled outright by some starter kits). Since 1.2.3 the package wires its own seeder calls into `database/seeders/DatabaseSeeder.php`, so a consumer whose seeder carried that trait was actively wiring the package into a configuration it could not survive: the insert failed with `SQLSTATE[HY000]: General error: 1364 Field 'public_id' doesn't have a default value` on MySQL/MariaDB (a NOT NULL constraint violation on SQLite), surfacing on the first `migrate --seed` or `migrate:refresh --seed` rather than during `install` itself. `HasPublicId` now generates `public_id` from Eloquent's `setUniqueIds()` unique-id hook instead of the `creating` event: `Model::performInsert()` calls `setUniqueIds()` directly — before firing `creating`, and not through the event dispatcher — so `public_id` is populated regardless of event suppression. The trait sets `$this->usesUniqueIds = true` from a new `initializeHasPublicId()` trait initializer (the same pattern Laravel's own `HasUniqueStringIds` uses) and calls `parent::setUniqueIds()` before filling `public_id`, so a model that also uses `HasUuids` or `HasUlids` still gets its own key populated. The old `creating`-event hook in `bootHasPublicId()` is unchanged and remains as a fallback for a model that turns `$usesUniqueIds` off. No consumer action is needed beyond `composer update` — there is no schema change and no new install step.
- This does not close every path to a missing `public_id`: `Model::upsert()` reads unique IDs through `Builder::addUniqueIdsToUpsertValues()`, a different code path this fix doesn't touch, so it still does not populate `public_id`; and `replicate()` still copies `public_id` onto the clone rather than clearing it, so saving a replicated model without reassigning its `public_id` violates the unique index. Closing either requires declaring `uniqueIds()`/`newUniqueId()` on `HasPublicId`, which is the trait-method collision with `HasUuids`/`HasUlids` this design deliberately avoids (see the design doc's §5 for the full reasoning). Documented as known limitations in the README and Boost skill rather than silently left uncovered.

### Changed
- `jamesgifford:auth:install` now detects, via a new `DatabaseSeederAnalysis::$usesWithoutModelEvents` flag resolved by `DatabaseSeederWiring::analyze()`, whether the consumer's `database/seeders/DatabaseSeeder.php` uses `WithoutModelEvents`, and prints an advisory warning naming what is actually at risk: model events are suppressed for the whole seeding run, including `AccountRole`'s guard against deleting a system role and any observers the consuming application registers — and explicitly stating that `public_id` generation is unaffected by the fix above. The warning fires from two independent call sites: after a successful seeder wire (`install`'s own wiring step), and from `install --verify`, which prints the same advisory as a `!` line rather than a failed check. Because `jamesgifford:auth:setup` always runs `jamesgifford:auth:install` internally as its Step 2 — regardless of whether `--with-dev-data` is passed — any `setup` run against such a consumer prints this warning too, via that nested install. `setup --with-dev-data` wires a second seeder, `DevDataSeeder`, through its own separate call site (`AuthSetupCommand::wireDevDataSeeder()`), which re-checks the same flag and can print the same warning a second time in one invocation — once from install's internal wiring step, once from the dev-data wiring step. The warning never fails an install and never flips `--verify`'s exit code; detection is best-effort (a single-file AST scan can't see a `WithoutModelEvents` use declared on a parent seeder class or on a seeder nested deeper than the one being analyzed).

### Development
- New tests drive the model-event-suppressed path directly, which every previous test avoided: `Model::withoutEvents(fn () => FixtureModel::create([...]))`, `$model->saveQuietly()` on a new model, and a `WithoutModelEvents` seeder that `$this->call()`s `DevDataSeeder` (the end-to-end reproduction of the reported failure) all now assert `public_id` is populated. A new fixture model combining `HasUuids` and `HasPublicId` guards against the rejected alternative design (declaring `uniqueIds()`/`newUniqueId()` in `HasPublicId`), which would be a fatal trait-method collision for any model using both.
- `DevDataSeeder`'s explicit-`public_id`-assignment fallback is now keyed on `usesUniqueIds()` rather than trait presence alone — a model can carry `HasPublicId` and still not auto-generate if `$usesUniqueIds` is disabled, and the old presence-only check would have kept passing even if the trait's generation were broken, which is exactly how this bug went undetected.
