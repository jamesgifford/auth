...
final class AuthSetupCommand extends Command
{
    ...
    private function educationalPause(): void
    {
        ...
        $this->line('low id range for seeded dev data). Declare them EITHER way — both are');
        $this->line('read, and the environment variable overrides the config value:');
        $this->newLine();
        $this->line('  • In config/jamesgifford/auth.php:');
        $this->newLine();
        ...
        $this->line('        ],');
        $this->newLine();
        $this->line('  • Or in your .env (overrides / supplements the config above):');
        $this->newLine();
